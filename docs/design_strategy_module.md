...
- **`update_market_data(df)` (Warm-up)**:
    - 전략 시작 전, 과거 데이터(예: 일봉, 최근 분봉)를 주입받아 지표를 초기화하거나 전일 레인지를 계산하는 용도.
    - 실시간 데이터 수신 전 필수적으로 호출됨.
...
- **체결 시뮬레이션**:
    ...
    - **슬리피지 (Slippage)**: 시장가 주문 시 최우선 호가보다 1~2틱 불리한 가격 체결 가정.
    - **지연 시간 (Latency)**: 주문 전송 후 체결까지의 네트워크/처리 지연(예: 100ms)을 시뮬레이션하여 체결 가능성 판단.
    - **시장 충격 (Market Impact)**: 대량 주문 시 호가 잔량을 소진시키고 가격을 변동시키는 효과 반영.
