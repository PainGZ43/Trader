...
class SystemMonitor:
    ...
    async def _check_network(self):
        ...
        # Check Kiwoom Server specifically
        await self._check_kiwoom_server()

    async def _check_kiwoom_server(self):
        """
        Check connectivity to Kiwoom OpenAPI Server.
        """
        def check_server():
            try:
                # openapi.kiwoom.com:80 (HTTP) or 443 (HTTPS)
                socket.create_connection(("openapi.kiwoom.com", 80), timeout=3)
                return True
            except OSError:
                return False

        loop = asyncio.get_running_loop()
        is_reachable = await loop.run_in_executor(None, check_server)
        
        if not is_reachable:
            self._publish_warning("Kiwoom Server Unreachable", "Failed to connect to openapi.kiwoom.com")
