from PyQt5.QtWidgets import (
    ...
    QDoubleSpinBox, QCheckBox, QTimeEdit, QPushButton, QMessageBox
)
...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.api_tab.setLayout(api_layout)
        ...
        self.strategy_tab.setLayout(strategy_layout)
        ...
        self.risk_tab.setLayout(risk_layout)
        ...
        self.notification_tab.setLayout(notif_layout)
        ...
        self.tab_widget.addTab(self.strategy_tab, "📊 전략")
        self.tab_widget.addTab(self.risk_tab, "⚠️ 리스크")
        self.tab_widget.addTab(self.notification_tab, "🔔 알림")
    ...
    def load_settings(self):
        ...
        try:
            ...
            # 전략 파라미터
            self.vol_threshold_spin.setValue(float(settings.get("VOL_THRESHOLD", 1.5)))
            self.ai_threshold_spin.setValue(float(settings.get("AI_THRESHOLD", 0.6)))
            self.take_profit_spin.setValue(float(settings.get("TAKE_PROFIT", 3.0)))
            self.stop_loss_spin.setValue(float(settings.get("STOP_LOSS", 2.0)))
            self.cooldown_spin.setValue(int(settings.get("COOLDOWN", 15)))
            
            # 리스크 관리
            self.max_positions_spin.setValue(int(settings.get("MAX_POSITIONS", 5)))
            self.max_position_size_spin.setValue(float(settings.get("MAX_POSITION_SIZE", 20.0)))
            self.daily_loss_limit_spin.setValue(float(settings.get("DAILY_LOSS_LIMIT", 5.0)))
            self.min_buy_amount_spin.setValue(int(settings.get("MIN_BUY_AMOUNT", 100000)))
            
            # 알림 설정
            self.notify_trade_check.setChecked(bool(settings.get("NOTIFY_TRADE", True)))
            self.notify_profit_check.setChecked(bool(settings.get("NOTIFY_PROFIT", True)))
            self.daily_report_check.setChecked(bool(settings.get("DAILY_REPORT", True)))
            
            report_time = settings.get("REPORT_TIME", "17:00")
            h, m = map(int, report_time.split(':'))
            self.report_time_edit.setTime(QTime(h, m))
            
            self.notify_error_check.setChecked(bool(settings.get("NOTIFY_ERROR", True)))
            
            # 고급 설정
            self.update_interval_spin.setValue(int(settings.get("UPDATE_INTERVAL", 5)))
            self.log_level_combo.setCurrentText(settings.get("LOG_LEVEL", "INFO"))
            self.enable_cache_check.setChecked(bool(settings.get("ENABLE_CACHE", True)))
            self.auto_reconnect_check.setChecked(bool(settings.get("AUTO_RECONNECT", True)))
            
        except Exception as e:
            logger.error(f"Failed to load settings: {e}")
            QMessageBox.warning(self, "설정 로드 오류", f"설정을 불러오는 중 오류가 발생했습니다:\n{e}\n기본값으로 표시됩니다.")
    ...
    def save_settings(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "저장 오류", f"설정을 저장하는 중 오류가 발생했습니다:\n{e}")
